...
//    Litecoin and modified for S88Coin.
//
//    Parameters: lookup (int) - How many blocks to look into the past.
//    Returns: Estimated S88Coin network hash rate (uint64_t)
//
...
// WM - Implementation of getnetworkhashps for S88Coin
Value getnetworkhashps(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "getnetworkhashps\n"
            "Returns an estimate of the S88Coin network hash rate.");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "S88Coin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "S88Coin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "S88Coin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "S88Coin is downloading blocks...");
